...
const Profile = () => {
  ...
  // Handle loading state or if agency data is not available
  ...
  // Safely get coordinates, if available
  const coordinates =
    agency.location && agency.location.coordinates
      ? [agency.location.coordinates[1], agency.location.coordinates[0]]
      : [0, 0]; // Default to [0, 0] if coordinates are not available
    ...
    <div className="w-full flex flex-col mb-20 items-center justify-center h-full scroll-smooth">
      <div className="w-full flex flex-col items-center gap-y-10 justify-center p-6">
        ...
        <Link to={"/change-password"}>
          ...
          >
            <button className="text-white overflow-hidden">Change Password</button>
            <MdPassword className="text-white" />
          </div>
        </Link>

        {/* Top box with edit profile button */}
